...
- Tests: the fork's suites (`npx jest src/utils/dijkstra src/utils/heap`) must be green. Two upstream suites
  fail on pristine checkouts too — `src/utils/graph.test.ts` (stale snapshots) and the controller spec; they
  are not yours to fix and not a regression signal.
